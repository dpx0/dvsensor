...
parser.add_argument('-i', required=True, dest='file_path', metavar="FILE",
					help='file path of the input file (must be a .csv)')
parser.add_argument('-rna', required=True, dest='mrna_path', metavar="FILE", help='mRNA sequence to be scored')
parser.add_argument('-o', required=True, dest='output_path', metavar="DIR",
					help='folder of the output file. Will be created if it doesn\'t exist yet')
...
with open(str(args.file_path), 'r') as csv_file:
	with open(str(args.mrna_path), 'r') as mrna_file:
		...
		for row in csv_reader:
			...
			output_file_name = str(current_path) + str(args.output_path) + "br_" + str(today) + "_" + str(
				file_counter) + ".xml"
			...
			blastx_cline = NcbiblastnCommandline(cmd=blastx_exe, query=query_file.name, subject=human_mrna,
												 out=output_file_name, outfmt=5)
			blastx_cline()
