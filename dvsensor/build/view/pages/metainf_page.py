...
def build(view, **kwargs) -> None:
	...
	if not view.page_allowed(kwargs):
		...
		return
	back_button('input', view, on_click=view.controller.cancel_task)
		...
		with ui.card().classes('no-shadow border-[1px] rounded-xl p-8 w-2/3 h-auto '):
			with ui.column().classes('w-full place-content-center'):
				...
				with ui.grid(columns=2).classes('place-self-start w-2/3'):
					...
					name_input.value = view.controller.get_model_data('rna_data.record_name')
					view.controller.bind_model_data(name_input, 'rna_data.record_name')

					ui.label('ID')\
						.classes('justify-self-end text-base font-mono font-semibold place-self-center mr-5')
					id_input = ui.input().classes('text-center text-base font-mono font-semibold')
					id_input.value = view.controller.get_model_data('rna_data.record_id')
					view.controller.bind_model_data(id_input, 'rna_data.record_id')

					ui.label('Description')\
						.classes('justify-self-end text-base font-mono font-semibold place-self-center mr-5')
					description_input = ui.input().classes('text-center text-base font-mono font-semibold')
					description_input.value = view.controller.get_model_data('rna_data.record_description')
					view.controller.bind_model_data(description_input, 'rna_data.record_description')
